    ...
    const fetchEvent = async () => {
      try {
        ...
        if (ev) {
          ...
          if (ev.endDate) {
            setEndDate(
              ev.endDate.toDate ? ev.endDate.toDate() : new Date(ev.endDate),
            );
          }
          ...
          Toast.show({
            type: 'error',
            text1: 'Not Found',
            text2: 'Event document does not exist',
          });
          navigation.goBack();
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      Toast.show({
        ...
        text2: 'Please fix highlighted fields',
      });
      return;
    }
    ...
    try {
      ...
      Toast.show({
        type: 'error',
        text1: 'Save Failed',
        text2: 'Firebase interaction failed.',
      });
    } finally {
      setSaving(false);
    }
          ...
          <View style={styles.dashboardHeader}>
            <View>
              ...
              <Text style={styles.dashboardSub}>
                Phase {totalProgress} of 5 Validated
              </Text>
            </View>
